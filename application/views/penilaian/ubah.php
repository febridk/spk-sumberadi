...
<body>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control" name="id_alternatif" id="id_alternatif">
                                                ...
                                                <?php foreach ($dataAlternatif as $data) { ?>
                                                    <option value="<?= $data['id_alternatif'] ?>" <?= (set_value('id_alternatif') ? set_value('id_alternatif') : $penilaian->id_alternatif) == $data['id_alternatif'] ? 'selected' : '' ?>><?= $data['nama_alternatif'] ?></option>
                                                <?php } ?>
                                            </select>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control" name="id_kriteria" id="id_kriteria">
                                                ...
                                                <?php foreach ($dataKriteria as $data) { ?>
                                                    <option value="<?= $data['id_kriteria'] ?>" <?= (set_value('id_kriteria') ? set_value('id_kriteria') : $penilaian->id_kriteria) == $data['id_kriteria'] ? 'selected' : '' ?>><?= $data['nama_kriteria'] ?></option>
                                                <?php } ?>
                                            </select>
    ...
    <script>
        ...
        $(document).ready(function() {
            var id_kriteria = '<?= $penilaian->id_kriteria ?>'
            var nama_subkriteria = '<?= $penilaian->nama_subkriteria ?>'
            var pilihanSubkriteria
            var subkriteriaTerpilih
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var loop = 0; loop < data.length; loop++) {
                        if (data[loop].nama_subkriteria == nama_subkriteria) {
                            subkriteriaTerpilih = 'selected'
                        } else {
                            subkriteriaTerpilih = ''
                        }

                        pilihanSubkriteria += '<option value="' + data[loop].nama_subkriteria + '" ' + subkriteriaTerpilih + '>' + data[loop].nama_subkriteria + '</option>';
                    }
            ...
            var nama_subkriteria = '<?= $penilaian->nama_subkriteria ?>'
            var id_subkriteria = '<?= $penilaian->id_subkriteria ?>'
            var pilihanNilai
            var nilaiTerpilih
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var loop = 0; loop < data.length; loop++) {
                        if (data[loop].id_subkriteria == id_subkriteria) {
                            nilaiTerpilih = 'selected'
                        } else {
                            nilaiTerpilih = ''
                        }

                        pilihanNilai += '<option value="' + data[loop].id_subkriteria + '" ' + nilaiTerpilih + '>' + data[loop].keterangan + '</option>';
                    }
